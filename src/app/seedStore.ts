...
export function seedLots(): AppLot[] {
  ...
  // Открытые слоты по другим странам — чтобы фильтр на /auctions менял ленту
  const trStart = new Date(Date.now() + 95 * 60 * 1000)
  merged.set('live-tr', {
    id: 'live-tr',
    ownerId: U_IMP,
    code: 'T-16',
    title: 'T-16 · текстиль Стамбул',
    isDraft: false,
    archived: false,
    settled: false,
    country: 'Турция',
    ...
    ready: isoToDdNow(),
    exportDecl: 'Платит отправитель',
    shipperName: 'Istanbul Tex',
    shipperAddress: 'Bagcilar',
    comment: 'авто, короче плечо',
    startIso: trStart.toISOString(),
    endIso: new Date(trStart.getTime() + 60 * 60 * 1000).toISOString(),
    included,
    createdAt: new Date().toISOString(),
    maxBidUsd: 3200,
    bids: [],
  })
  const vnStart = new Date(Date.now() + 3 * 60 * 60 * 1000)
  merged.set('live-vn', {
    id: 'live-vn',
    ownerId: U_IMP,
    code: 'VN-04',
    title: 'VN-04 · пошив Хошимин',
    isDraft: false,
    archived: false,
    settled: false,
    country: 'Вьетнам',
    ...
    ready: isoToDdNow(),
    exportDecl: 'Платит отправитель',
    shipperName: 'HCM Softgoods',
    shipperAddress: 'District 7',
    comment: 'FOB, море+жд',
    startIso: vnStart.toISOString(),
    endIso: new Date(vnStart.getTime() + 60 * 60 * 1000).toISOString(),
    included,
    createdAt: new Date().toISOString(),
    maxBidUsd: 5400,
    bids: [],
  })
  const inStart = new Date(Date.now() + 5 * 60 * 60 * 1000)
  merged.set('live-in', {
    id: 'live-in',
    ownerId: U_IMP,
    code: 'IN-03',
    title: 'IN-03 · ткани Мумбаи',
    isDraft: false,
    archived: false,
    settled: false,
    country: 'Индия',
    ...
    shipperAddress: 'Bhiwandi',
    comment: 'рулоны, FOB',
    startIso: inStart.toISOString(),
    endIso: new Date(inStart.getTime() + 60 * 60 * 1000).toISOString(),
    included,
    createdAt: new Date().toISOString(),
    maxBidUsd: 6100,
    bids: [],
